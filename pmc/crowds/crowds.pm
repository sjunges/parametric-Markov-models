...
// Model parameters
const double pK;  // forwarding probability
const double pL;  // probability that member is untrustworthy

// Probability of forwarding
// const double    pK = 0.8;
// const double notpK = 0.2;  // must be 1-pK

// Probability that a crowd member is bad
// const double  pL = 0.1;
// const double  pL = 0.091;
// const double  pL = 0.167;
// const double goodC = 0.909;  // must be 1-pL
// const double goodC = 0.833;  // must be 1-pL
...
module crowds
	...
	[] !good & !bad & !deliver & run ->
	              1-pL : (good'=true) & (recordLast'=true) & (run'=false) +
	               pL : (bad'=true)  & (badObserve'=true) & (run'=false);

	// GOOD MEMBERS
	// Forward with probability pK, else deliver
	[] good & !deliver & run -> pK : (good'=false) + 1-pK : (deliver'=true);
